...
export default function ServiceRequestsBoard({type,title,description,meetings=false}:{type:"contract_review"|"contract_drafting";title:string;description:string;meetings?:boolean}){
  ...
  async function submitStatus(event:FormEvent<HTMLFormElement>){
    ...
    try{
      ...
      setStatusDialog(null); setNotice("تم تحديث حالة الطلب."); await load();
  ...
  return <div className="space-y-6">
    ...
    <AdminDialog open={Boolean(meetingDialog)} title="تحديد موعد مناقشة المراجعة" description="اختر من المواعيد الفعلية المتاحة حسب جدول المكتب والسعة. العميل سيصله إشعار بعد الحفظ." onClose={()=>!busy&&setMeetingDialog(null)}>
