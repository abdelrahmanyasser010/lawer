...
type StatusDialogState = { id:number; status:string; notes:string; visibleToClient:boolean } | null;
  ...
  return <div className="space-y-6">
    ...
    <AdminDialog open={Boolean(statusDialog)} title="تحديث حالة الاستشارة" description="اختر الحالة الجديدة، وأضف ملاحظة عند الحاجة. يمكنك تحديد ما إذا كانت الملاحظة ستظهر للعميل." onClose={()=>!busy&&setStatusDialog(null)}>
      {statusDialog&&<form onSubmit={submitStatus} className="space-y-4">
        <label className="block"><span className="mb-2 block text-xs font-black text-slate-600">الحالة</span><select value={statusDialog.status} onChange={e=>setStatusDialog({...statusDialog,status:e.target.value})} className="w-full rounded-xl border border-slate-200 p-3 text-sm">{selectableStatuses.map(s=><option key={s} value={s}>{statusLabels[s]??s}</option>)}</select></label>
