  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="border-b border-white/10 px-4 py-4">
        <Link href="/account" onClick={onNavigate} className="block rounded-2xl border border-white/10 bg-white/5 p-3 transition hover:bg-white/10">
          <div className="flex items-center gap-2.5">
            <div className="flex h-9 w-9 items-center justify-center rounded-full bg-[#986410]/20 text-xs font-black text-[#d7a94f]">{staff.name.slice(0, 1)}</div>
            <div className="min-w-0"><div className="truncate text-xs font-black text-white">{staff.name}</div><div className="truncate text-[10px] text-slate-400">{staff.email}</div></div>
          </div>
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4">
        {visibleGroups.map((group) => (
          <div key={group.label} className="mb-5">
            <div className="mb-2 px-3 text-[9px] font-black uppercase tracking-wider text-slate-500">{group.label}</div>
            <div className="space-y-1">
              {group.items.map((item) => {
                const Icon = item.icon;
                const active = isActive(pathname, item.href);
                return (
                  <Link key={item.href} href={item.href} onClick={onNavigate} className={`group flex items-center gap-3 rounded-xl px-3 py-2.5 text-xs font-bold transition ${active ? "bg-[#986410] text-white shadow-sm" : "text-slate-300 hover:bg-white/8 hover:text-white"}`}>
                    <Icon className={`h-4 w-4 shrink-0 ${active ? "text-white" : "text-slate-400 group-hover:text-[#d7a94f]"}`} />
                    <span className="flex-1">{item.label}</span>
                    {!!item.badge && <span className={`rounded-full px-2 py-0.5 text-[9px] font-black ${active ? "bg-white text-[#986410]" : "bg-rose-500 text-white"}`}>{item.badge}</span>}
                    {active && <ChevronLeft className="h-3 w-3 text-white/60" />}
                  </Link>
                );
              })}
            </div>
      ...
      <div className="border-t border-white/10 p-3">
        <button type="button" onClick={async () => { try { await dashboardApi.logout(); } finally { setDashboardUser(null); window.location.href = "/login"; } }} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-xs font-bold text-slate-400 hover:bg-white/8 hover:text-white"><LogOut className="h-4 w-4" /> تسجيل الخروج</button>
      </div>
    </div>
  );
}
...
export default function AdminSidebar() {
  ...
  return (
    <>
      ...
      {open && <div className="fixed inset-0 z-50 lg:hidden"><button type="button" aria-label="إغلاق" className="absolute inset-0 bg-black/50" onClick={() => setOpen(false)} /><aside className="absolute inset-y-0 right-0 w-[min(88vw,20rem)] bg-[#00102e] shadow-2xl"><button type="button" onClick={() => setOpen(false)} className="absolute left-4 top-4 rounded-lg p-2 text-white hover:bg-white/10" aria-label="إغلاق القائمة"><X className="h-4 w-4" /></button><div className="h-full pt-10"><SidebarBody pathname={pathname} onNavigate={() => setOpen(false)} /></div></aside></div>}
    </>
  );
}
