      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-2">
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-5 space-y-3">
            ...
            <label className="block"><span className="mb-1 block text-xs font-black text-slate-700">كلمة المرور الجديدة</span><input type="password" minLength={8} value={newPassword} onChange={(event)=>setNewPassword(event.target.value)} className="w-full rounded-xl border border-slate-300 px-3.5 py-2.5 text-xs outline-none focus:border-[#986410]" /></label>
            <label className="block"><span className="mb-1 block text-xs font-black text-slate-700">تأكيد كلمة المرور</span><input type="password" minLength={8} value={confirmPassword} onChange={(event)=>setConfirmPassword(event.target.value)} className="w-full rounded-xl border border-slate-300 px-3.5 py-2.5 text-xs outline-none focus:border-[#986410]" /></label>
          </div>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 space-y-3">
            {sessions.map((session) => {
              const MobileIcon = /Android|iPhone|iPad/i.test(session.user_agent || "") ? Smartphone : Laptop2;
              return <div key={session.id} className={`flex items-center justify-between gap-3 rounded-xl border p-3 ${session.current ? "border-emerald-200 bg-emerald-50" : "border-slate-200 bg-slate-50"}`}><div className="flex items-center gap-3"><MobileIcon className={`h-5 w-5 ${session.current ? "text-emerald-700" : "text-slate-500"}`} /><div><div className={`text-xs font-black ${session.current ? "text-emerald-900" : "text-slate-800"}`}>{deviceLabel(session.user_agent)}</div><div className="text-[10px] font-bold text-slate-400">{session.ip_address || "IP غير متاح"} · {session.last_seen_at ? new Date(session.last_seen_at).toLocaleString("ar-EG") : "نشاط غير مسجل"}</div></div></div>{session.current ? <span className="rounded-full bg-white px-2 py-1 text-[9px] font-black text-emerald-700">حالي</span> : <button type="button" onClick={()=>void revoke(session.id)} className="inline-flex items-center gap-1 rounded-lg border border-rose-200 bg-white px-3 py-2 text-[10px] font-black text-rose-700"><LogOut className="h-3.5 w-3.5" /> إلغاء</button>}</div>;
            })}
          ...
          <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-3 text-[11px] font-bold leading-6 text-amber-900"><KeyRound className="ml-1 inline h-4 w-4" />الأساس مجهز لإضافة 2FA لاحقًا للسوبر أدمن والمحامين دون تغيير نظام الجلسات.</div>
        </section>
      </div>
    </div>
  );
