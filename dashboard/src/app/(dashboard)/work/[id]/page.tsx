...
const selectableStatuses = dashboardFeatures.assignment ? statuses : statuses.filter((status) => status !== "assigned");
...
type Details = {
  ...
  lawyerRemainingEgp?: number | null; approvedPaidEgp?: number; outstandingEgp?: number; paymentStage?: string | null;
...
export default function WorkRequestDetailsPage() {
  ...
  useEffect(() => {
    if (!data || data.requestType !== "consultation" || !canSchedule) {
  ...
  return <div className="mx-auto max-w-7xl space-y-6 p-6 sm:p-8">
    <header className="flex flex-col justify-between gap-4 border-b border-slate-200 pb-5 lg:flex-row lg:items-start">
      ...
      <div className="flex flex-wrap gap-2"><button onClick={() => void load()} className="rounded-xl border border-slate-200 bg-white p-2.5"><RefreshCw className="h-4 w-4" /></button>{canAssign && <select disabled={busy} value={data.assignedLawyerId ?? ""} onChange={(event) => event.target.value && void assign(Number(event.target.value))} className="rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-xs font-black"><option value="">اختيار المسؤول</option>{activeLawyers.map((lawyer) => <option key={lawyer.id} value={lawyer.id}>{lawyer.name}</option>)}</select>}{canManage && <button type="button" disabled={busy} onClick={() => setStatusDialog({ status: (!dashboardFeatures.assignment && data.status === "assigned") ? "in_progress" : data.status, notes: "", visibleToClient: true })} className="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-black text-[#00102e]">تحديث الحالة</button>}</div>
    </header>
    ...
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      ...
      {data.requestType === "contract_drafting" && data.lawyerTotalPriceEgp != null
        ? <FinancialCard total={Number(data.lawyerTotalPriceEgp)} deposit={Number(data.lawyerDepositEgp ?? 0)} paid={Number(data.approvedPaidEgp ?? 0)} remaining={Number(data.outstandingEgp ?? data.lawyerRemainingEgp ?? 0)} status={data.paymentStatus || undefined} />
    ...
    <div className="grid gap-5 xl:grid-cols-[1.35fr_1fr]">
      ...
      <div className="space-y-5">
        ...
        {canSchedule && <form onSubmit={scheduleMeeting} className="rounded-2xl border border-slate-200 bg-white p-5">
          <div className="flex items-center gap-2"><CalendarPlus className="h-4 w-4 text-[#986410]" /><div><h2 className="font-black text-[#00102e]">تحديد موعد التواصل</h2><p className="mt-1 text-[10px] text-slate-400">الاستشارات تستخدم فقط المواعيد المتاحة في جدول المكتب وتراعي السعة والحظر.</p></div></div>
          ...
          <button disabled={busy || (data.requestType === "consultation" && (!selectedMeetingStart || availabilityLoading))} className="mt-4 rounded-xl bg-[#00102e] px-4 py-2.5 text-xs font-black text-[#d5a84c] disabled:cursor-not-allowed disabled:opacity-50">حفظ الموعد وإشعار العميل</button>
        </form>}
      </div>
    </div>
    ...
    <AdminDialog open={Boolean(statusDialog)} title="تحديث حالة الطلب" description="اختر الحالة الجديدة وأضف ملاحظة عند الحاجة. يمكنك إبقاء الملاحظة داخلية أو إظهارها للعميل." onClose={() => !busy && setStatusDialog(null)}>
      {statusDialog && <form onSubmit={submitStatus} className="space-y-4">
        <label className="block"><span className="mb-2 block text-xs font-black text-slate-600">الحالة</span><select value={statusDialog.status} onChange={(event) => setStatusDialog({ ...statusDialog, status: event.target.value })} className="w-full rounded-xl border border-slate-200 p-3 text-sm">{selectableStatuses.map((status) => <option key={status} value={status}>{statusLabels[status] ?? status}</option>)}</select></label>
