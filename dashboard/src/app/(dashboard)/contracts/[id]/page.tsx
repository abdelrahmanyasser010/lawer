...
type ContractDetails = {
  ...
  billing_mode?: string;
  versions: Array<{ id: number; versionNumber: number; status: string; createdAt: string; lockedAt?: string; documentHash?: string }>;
};
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-[1.3fr_.7fr]">
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            {[
              ["الحالة", contract.status], ["نوع القالب", contract.template_name_ar], ["العميل", contract.client_name || "استخدام داخلي"], ...(dashboardFeatures.assignment ? [["المحامي", contract.assigned_lawyer_name || "غير مسند"]] : []), ["مصدر الإنشاء", contract.source_channel], ["نظام التحصيل", contract.billing_mode],
        ...
        <aside className="space-y-4">
          ...
          {hasPermission("contracts.manage_status") && <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-sm font-black text-[#00102e]">تغيير سير العمل</h2><select value={status} onChange={(event)=>setStatus(event.target.value)} className="mt-4 w-full rounded-xl border border-slate-300 px-3 py-2.5 text-xs font-bold"><option value="pending_review">بانتظار المراجعة</option><option value="assigned">قيد المتابعة</option><option value="in_progress">قيد الصياغة</option><option value="client_review">مراجعة العميل</option><option value="revision_requested">تعديلات مطلوبة</option><option value="approved">معتمد</option><option value="cancelled">ملغي</option></select><button type="button" onClick={()=>void action("/status",{status})} className="mt-3 w-full rounded-xl border border-slate-300 py-2.5 text-xs font-black text-slate-700">حفظ الحالة</button></section>}

          {hasPermission("contracts.waive_payment") && <section className="rounded-2xl border border-amber-200 bg-amber-50 p-5"><h2 className="text-sm font-black text-amber-950">إعفاء من الدفع</h2><textarea value={waiverReason} onChange={(event)=>setWaiverReason(event.target.value)} placeholder="سبب واضح ومسجل في Audit Log" className="mt-3 min-h-20 w-full rounded-xl border border-amber-200 bg-white p-3 text-xs outline-none"/><button type="button" disabled={!waiverReason.trim()} onClick={()=>void action("/payment-waiver",{reason:waiverReason})} className="mt-3 w-full rounded-xl bg-amber-900 py-2.5 text-xs font-black text-white disabled:opacity-50">تسجيل الإعفاء</button></section>}
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <button type="button" onClick={()=>void createShareLink()} className="mt-3 w-full rounded-xl bg-[#00102e] py-2.5 text-xs font-black text-white">إنشاء رابط لمدة 7 أيام</button>
