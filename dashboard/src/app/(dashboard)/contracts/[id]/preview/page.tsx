...
export default function StaffContractPreviewPage() {
  ...
  const resolved = useMemo(() => {
    ...
    try {
      return resolveWizardDefinition(payload.templateDefinition, payload.variantKey, payload.selectedOptionalClauseKeys, payload.fieldValues as any);
    } catch {
      return null;
    }
  }, [payload]);
  ...
  const clauses = useMemo<LegalClause[]>(() => {
    ...
    try {
      return renderLegalClauses(payload.templateDefinition, payload.variantKey, payload.selectedOptionalClauseKeys, payload.fieldValues as any) as LegalClause[];
    } catch {
      return [];
    }
  }, [payload]);
