...
import { ArrowLeft, Building2, Check, CheckCircle2, CreditCard, Phone, Scale, Upload, User, Video, X } from "lucide-react";
...
const channelOptions: Array<{ key: CommunicationChannel; label: string; icon: typeof Building2 }> = [
  { key: "office", label: "مقابلة في المكتب", icon: Building2 },
...
export default function CreationModeModal({ template: initialTemplate, isOpen, onClose }: Props) {
  ...
  async function submitLawyerRequest(event: React.FormEvent) {
    ...
    if (!cashNumber) {
      setError("رقم الدفع غير مضبوط بعد من إدارة المكتب.");
      return;
    }
    if (!receipt) {
    ...
    try {
      ...
      try {
        await apiRequest("/api/v1/payments/receipts", {
          method: "POST",
          body: JSON.stringify({ serviceRequestId: request.id, amountEgp: draftingDeposit, senderPhone: phone, attachmentId: attachment.id }),
        });
      ...
      <section className="relative max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-3xl border border-slate-700 bg-slate-950 p-6 text-right shadow-2xl sm:p-8">
          ...
          <div>
            ...
            <div className="mt-7 grid gap-4 sm:grid-cols-2">
              ...
              <button type="button" onClick={() => setStep("lawyer")} className="rounded-2xl border border-[#986410]/50 bg-[#986410]/10 p-5 text-right transition hover:border-[#d9a84e]">
                <div className="flex items-start justify-between gap-3"><div className="rounded-xl bg-[#986410]/15 p-3 text-[#d9a84e]"><Scale className="h-5 w-5" /></div><span className="text-xs font-black text-[#d9a84e]">عربون {draftingDeposit} ج.م</span></div>
                <h3 className="mt-5 text-lg font-black text-white">يتولى محامي المكتب الإعداد</h3>
                <p className="mt-2 text-xs leading-6 text-slate-300">اختر طريقة التواصل وارفع إثبات العربون. بعد المراجعة يُسند الطلب لمحامٍ، وتظهر النسخ الجاهزة داخل حسابك.</p>
        ...
        {step === "lawyer" && (
          <form onSubmit={submitLawyerRequest} className="space-y-5">
            ...
            <div className="rounded-2xl border border-slate-700 bg-slate-900 p-4 text-xs leading-6 text-slate-300">
              <div className="flex items-center justify-between"><span className="inline-flex items-center gap-2"><CreditCard className="h-4 w-4 text-[#d9a84e]" /> عربون الحجز</span><strong className="text-white">{draftingDeposit} ج.م</strong></div>
              <p className="mt-2">حوّل المبلغ إلى رقم Vodafone Cash التالي، ثم ارفع صورة أو PDF لإثبات التحويل. يتم تأكيد الطلب بعد مراجعة الإدارة.</p>
              <div className="mt-3 rounded-xl border border-[#986410]/30 bg-slate-950 px-4 py-3 text-center font-mono text-base font-black text-[#d9a84e]" dir="ltr">{cashNumber || "غير محدد بعد"}</div>
            </div>

            <label className="block cursor-pointer rounded-2xl border border-dashed border-slate-600 bg-slate-900 p-5 text-center">
            ...
            <button disabled={submitting || !cashNumber} className="w-full rounded-xl bg-[#986410] py-3.5 text-sm font-black text-white disabled:opacity-60">{submitting ? "جاري إرسال الطلب..." : "إرسال الطلب وإثبات الدفع"}</button>
            {!cashNumber && <p className="text-center text-[10px] font-bold text-red-300">الخدمة متوقفة مؤقتًا حتى تضبط الإدارة رقم الدفع.</p>}
          </form>
        )}
        ...
        {step === "success" && (
          <div className="py-5 text-center">
            ...
            <p className="mt-2 text-sm leading-7 text-slate-300">مرجع الطلب: <strong className="font-mono text-[#d9a84e]">{reference}</strong>. {paymentSubmitted ? "تم استلام إثبات الدفع ويمكنك متابعة مراجعته وإسناد الطلب من حسابك." : "تم حفظ الطلب، لكن إثبات الدفع لم يكتمل. يمكنك رفعه من صفحة الطلب بدون إنشاء طلب جديد."}</p>
