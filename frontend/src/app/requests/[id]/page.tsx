...
function RequestDetailsContent() {
  ...
  async function requestRevision() {
    ...
    try {
      ...
      setRevisionNote("");
      setNotice("تم تسجيل طلب التعديل وسيظهر للمحامي المسؤول.");
      await load();
    } catch (caught) {
  ...
  async function confirmReceipt() {
    if (!window.confirm("هل تؤكد أنك راجعت النسخة المتاحة ولا تحتاج إلى تعديلات أخرى؟")) return;
    setBusy(true);
    setError("");
    try {
  ...
  const ContactIcon = item.communicationChannel === "office" ? Building2 : item.communicationChannel === "zoom" ? Video : MessageCircle;
  const whatsappDigits = catalog.office.whatsappNumber.replace(/\D/g, "").replace(/^0/, "20");
  const whatsappUrl = whatsappDigits
    ? `https://wa.me/${whatsappDigits}?text=${encodeURIComponent(`مرحبًا، أتابع الطلب ${item.serialNumber}`)}`
    : "";
  const serviceDeposit = item.requestType === "contract_review"
    ? catalog.services.contractReviewDepositEgp
    : item.requestType === "consultation"
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-6">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-xs text-slate-500">ارفع المستندات المطلوبة فقط. لا توجد محادثات داخل المنصة.</p>
                </div>
            ...
            <section className="rounded-2xl border border-emerald-200 bg-white p-6">
              ...
              {item.permissions.canRequestRevision && (
                <div className="mt-5 border-t border-slate-200 pt-5">
                  ...
                  <textarea value={revisionNote} onChange={(event) => setRevisionNote(event.target.value)} rows={3} placeholder="اكتب التعديلات المطلوبة باختصار، أو تواصل مع المحامي بالطريقة المحددة للطلب." className="mt-2 w-full rounded-xl border border-slate-300 p-3 text-sm outline-none focus:border-[#986410]" />
              ...
              {item.permissions.canConfirmReceipt && (
                <button type="button" disabled={busy} onClick={() => void confirmReceipt()} className="mt-5 inline-flex items-center gap-2 rounded-xl bg-emerald-700 px-5 py-3 text-xs font-black text-white disabled:opacity-50"><CheckCircle2 className="h-4 w-4" /> تأكيد استلام ومراجعة النسخة</button>
              )}
            </section>
          </div>
          ...
          <aside className="space-y-6">
            <section className="rounded-2xl border border-slate-200 bg-white p-6">
              ...
              <dl className="mt-4 space-y-3 text-xs">
                <div className="flex justify-between gap-3"><dt className="text-slate-500">المحامي المسؤول</dt><dd className="font-black text-slate-800">{item.assignedLawyerName || "لم يتم الإسناد بعد"}</dd></div>
                <div className="flex justify-between gap-3"><dt className="text-slate-500">الموعد</dt><dd className="font-black text-slate-800">{formatDate(item.meetingAt || item.preferredContactAt)}</dd></div>
                {item.meetingLocation && <div><dt className="text-slate-500">عنوان المكتب</dt><dd className="mt-1 font-bold leading-6 text-slate-800">{item.meetingLocation}</dd></div>}
              </dl>
              ...
              {item.communicationChannel === "office" && !item.meetingLocation && catalog.office.address && <div className="mt-4 rounded-xl bg-slate-50 p-3 text-xs leading-6 text-slate-700"><b>عنوان المكتب:</b> {catalog.office.address}</div>}
            </section>

            <section className="rounded-2xl border border-slate-200 bg-white p-6">
              <h2 className="font-black text-[#00102e]">الدفع</h2>
              <div className="mt-4 text-sm font-black text-slate-800">{paymentStatusLabels[item.paymentStatus || ""] || "لم تُسجل عملية دفع"}</div>
              <p className="mt-1 text-xs text-slate-500">المبلغ المطلوب: {item.paymentAmountEgp ?? serviceDeposit} ج.م</p>
              {item.paymentAdminNotes && <div className="mt-3 rounded-xl border border-red-200 bg-red-50 p-3 text-xs font-bold leading-6 text-red-700">{item.paymentAdminNotes}</div>}
              {canSubmitPayment && <button type="button" onClick={() => setPaymentOpen(true)} className="mt-4 w-full rounded-xl bg-[#00102e] px-4 py-3 text-xs font-black text-white">{item.paymentStatus === "rejected" ? "رفع إثبات دفع جديد" : "رفع إثبات الدفع"}</button>}
            </section>

            {item.linkedContractId && (
              <section className="rounded-2xl border border-blue-200 bg-blue-50 p-6">
                <h2 className="font-black text-blue-950">العقد الذي يعده المكتب</h2>
