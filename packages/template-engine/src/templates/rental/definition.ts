...
function partyFields(prefix: "landlord" | "tenant", label: string): WizardFieldDefinition[] {
  ...
  return [
    ...
    { key: `${prefix}_identity_document_type`, type: "radio", labelAr: "نوع مستند إثبات الهوية", visibleWhen: individualCondition, requiredWhen: individualCondition, printInDocument: false, options: identityDocumentOptions },
...
.5 انتقال ملكية العين المؤجرة: إذا انتقلت ملكية العين إلى الغير أثناء سريان العقد، فلا يترتب على ذلك بذاته انتهاء العقد، ويحل الخلف الخاص محل المؤجر في الحقوق والالتزامات في الحدود المقررة قانونًا مع مراعاة الحقوق المكتسبة للمستأجر.`;
  ...
  if (key === "residential_lease_source_article_07") {
    body = `.1 مبلغ التأمين: اتفق الطرفان على مبلغ تأمين قدره {{deposit_amount}} جنيهًا مصريًا، فقط ({{deposit_amount_words}} جنيهًا مصريًا لا غير). {{rental_deposit_receipt_text}} ويخصص هذا المبلغ لضمان تنفيذ التزامات المستأجر.
...
.5 عدم اعتبار مبلغ التأمين أجرة: لا يجوز للمستأجر اعتبار مبلغ التأمين وفاءً بالأجرة أو بأي قسط منها، ويظل ملتزمًا بسداد الأجرة حتى انتهاء العلاقة الإيجارية وتسوية الالتزامات.`;
  ...
  if (key === "commercial_lease_source_article_07") {
    body = `.1 مبلغ التأمين: اتفق الطرفان على مبلغ تأمين قدره {{deposit_amount}} جنيهًا مصريًا، فقط ({{deposit_amount_words}} جنيهًا مصريًا لا غير). {{rental_deposit_receipt_text}} ويخصص هذا المبلغ لضمان تنفيذ جميع التزامات المستأجر الناشئة عن العقد.
...
.6 عدم جواز اعتبار مبلغ التأمين أو شيكات الضمان وفاءً: يظل كل منهما - إن وجد - ضمانًا لتنفيذ الالتزامات ولا يعفي المستأجر من سداد الأجرة وسائر المستحقات في مواعيدها.`;
  ...
  if (key === "administrative_lease_source_article_07") {
    body = `.1 مبلغ التأمين: اتفق الطرفان على مبلغ تأمين قدره {{deposit_amount}} جنيهًا مصريًا، فقط ({{deposit_amount_words}} جنيهًا مصريًا لا غير). {{rental_deposit_receipt_text}} ويخصص هذا المبلغ لضمان تنفيذ جميع الالتزامات الناشئة عن العقد.
