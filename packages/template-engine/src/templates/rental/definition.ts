...
.6 عدم جواز اعتبار مبلغ التأمين أو شيكات الضمان وفاءً: لا يعفي وجود أي منهما المستأجر من سداد الأجرة وسائر الالتزامات المالية في مواعيدها.`;
  ...
  if (["residential_lease_source_article_22", "commercial_lease_source_article_21", "administrative_lease_source_article_21"].includes(key)) {
    body = "يقر الطرفان بأنهما اطلعا على العقد ومالحقه ومحاضره – إن وجدت – وفهما الحقوق والالتزامات الناشئة عنه، وأن توقيعهما عليه تم بإرادتهما الحرة وبعد التحقق من بياناته. وتثبت بيانات التوقيع والبصمة والشهود – إن وجدوا – في قسم التوقيعات المخصص بنهاية الوثيقة، ويقتصر توقيع الشاهد على إثبات حضور واقعة التوقيع ما لم يوقع بصفة قانونية أخرى صراحةً.";
  }
...
const conditionalClauses: LegalClauseDefinition[] = [
  ...
  customClause({ key: "rental_commercial_vat_clause", titleAr: "ضريبة القيمة المضافة (فقرة اختيارية)", bodyAr: sliceRange(sourceBody("commercial_lease_source_article_08"), "(فقرة اختيارية – إذا كانت األجرة خاضعة لضريبة القيمة المضافة)"), visibleWhen: { fieldKey: "commercial_vat_enabled", operator: "truthy" }, sourceDocumentName: "عقد ايجار تجاري ( Z DRAFT ).pdf", sourcePageStart: 7, sourcePageEnd: 7 }),
  ...
  customClause({ key: "rental_commercial_signage_clause", titleAr: "اللافتات والعلامات التجارية والإعلانات (فقرة اختيارية)", bodyAr: sliceRange(sourceBody("commercial_lease_source_article_11"), "(فقرة اختيارية – الالفتات والعالمات التجارية واإلعالنات)", ".5إزالة األعمال المخالفة"), visibleWhen: { fieldKey: "commercial_signage_enabled", operator: "truthy" }, sourceDocumentName: "عقد ايجار تجاري ( Z DRAFT ).pdf", sourcePageStart: 10, sourcePageEnd: 10 }),
  ...
  customClause({ key: "rental_administrative_safety_clause", titleAr: "اشتراطات السلامة والدفاع المدني (فقرة اختيارية)", bodyAr: sliceRange(sourceBody("administrative_lease_source_article_06"), "(فقرة اختيارية – إذا كان النشاط يستلزم اشتراطات خاصة للسالمة أو الدفاع المدني)"), visibleWhen: { fieldKey: "administrative_safety_enabled", operator: "truthy" }, sourceDocumentName: "إداري عقد ايجار Z DRAFT().pdf", sourcePageStart: 5, sourcePageEnd: 6 }),
  ...
  customClause({ key: "rental_administrative_vat_clause", titleAr: "ضريبة القيمة المضافة (فقرة اختيارية)", bodyAr: sliceRange(sourceBody("administrative_lease_source_article_08"), "(فقرة اختيارية – إذا كانت األجرة خاضعة لضريبة القيمة المضافة)"), visibleWhen: { fieldKey: "administrative_vat_enabled", operator: "truthy" }, sourceDocumentName: "إداري عقد ايجار Z DRAFT().pdf", sourcePageStart: 8, sourcePageEnd: 8 }),
  ...
  customClause({ key: "rental_administrative_signage_clause", titleAr: "اللافتات ولوحات التعريف بالشركة (فقرة اختيارية)", bodyAr: sliceRange(sourceBody("administrative_lease_source_article_11"), "(فقرة اختيارية – الالفتات ولوحات التعريف بالشركة)", ".5إزالة األعمال المخالفة"), visibleWhen: { fieldKey: "administrative_signage_enabled", operator: "truthy" }, sourceDocumentName: "إداري عقد ايجار Z DRAFT().pdf", sourcePageStart: 11, sourcePageEnd: 11 }),
  ...
  customClause({ key: "rental_administrative_esign_clause", titleAr: "التوقيع الإلكتروني والمنصات الرقمية (إن وجد)", bodyAr: sliceRange(sourceBody("administrative_lease_source_article_18"), ".7التوقيع اإللكتروني والمنصات الرقمية"), visibleWhen: { fieldKey: "administrative_esign_enabled", operator: "truthy" }, sourceDocumentName: "إداري عقد ايجار Z DRAFT().pdf", sourcePageStart: 21, sourcePageEnd: 21 }),
