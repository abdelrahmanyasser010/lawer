...
return new class extends Migration {
    public function up(): void
    {
        ...
        if ($canonicalVersion !== $targetVersion) {
            throw new RuntimeException('Canonical freelancer v14 definition is required by this migration.');
        }
        ...
        DB::transaction(function () use ($raw, $definition, $targetVersion): void {
            ...
            if ($version) {
                ...
                if ($existing != $definition) {
                    throw new RuntimeException('freelancer template version 14 already exists with a different immutable definition.');
                }
                ...
                $versionId = DB::table('template_versions')->insertGetId([
                    ...
                    'status' => 'published',
                    'definition_json' => $raw,
                    'change_summary' => 'مراجعة شاملة لعقد إدارة حسابات ومنصات التواصل الاجتماعي: إزالة بيانات مشروعات الويب المنسوخة، وربط بيانات الأطراف والنشاط والحسابات والمنصات ونطاق الخدمات والمحتوى والحملات الإعلانية والتقارير وKPIs والمدة والمقابل والسداد والمراجعات والملفات الأصلية وPortfolio والجزاء ومدد الإنهاء والقوة القاهرة والإخطارات والمحكمة داخل المواد القانونية، مع فحص تغطية Fail-Closed لكل حقول الـWizard.',
                    'legal_reference' => 'Reviewed against عقد تقديم خدمات إدارة حسابات ومنصات التواصل الاجتماعي(1).pdf (26 pages), approved 2026-08-19',
                    'effective_from' => now(),
                    'published_at' => now(),
                    'created_at' => now(),
                    'updated_at' => now(),
