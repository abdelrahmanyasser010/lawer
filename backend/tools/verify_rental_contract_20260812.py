...
check('rental_version_12',d.get('version')==12)
...
# Handover/inventory annex is faithfully blank/manual and always optional.
...
check('handover_blank_manual_description','قالب فارغ للطباعة والتعبئة اليدوية' in ann.get('description','') and 'لا تُنقل إليه بيانات الـWizard' in ann.get('description',''))
check('handover_all_three_variants',set(ann.get('applicableVariantKeys',[]))==set(variants))
check('handover_never_mandatory','requiredWhen' not in ann)
check('handover_never_auto_added','لا يُضاف تلقائيًا' in ann.get('description',''))
...
check('frontend_annexes_are_explicit_only','requiredWhen' not in optui and 'selectedClauseKeys.includes(clause.key)' in optui)
check('wizard_annexes_are_explicit_only','requiredAnnexKeys' not in wizard and 'draft?.selectedOptionalClauseKeys.includes(clause.key)' in wizard)
check('backend_annexes_are_explicit_only','Every annex is optional' in php and "array_map('strval',$selected)" in php and 'requiredAnnexKeys' not in php)
check('pdf_blank_rows_support','blankRows' in pdf and 'blankRowLabels' in pdf)
check('pdf_selected_annexes_only','Every annex is optional' in pdf and "array_map('strval',$selected)" in pdf and 'requiredAnnexKeys' not in pdf)
...
mig9=txt('backend/database/migrations/2026_08_18_000100_publish_contract_integrity_versions.php')
mig10=txt('backend/database/migrations/2026_08_18_000300_publish_rental_sale_text_integrity_versions.php')
mig11=txt('backend/database/migrations/2026_08_18_000400_publish_blank_annex_template_versions.php')
mig12=txt('backend/database/migrations/2026_08_18_000500_publish_optional_annex_template_versions.php')
...
check('migration_v8_newer_guard',"$canonicalVersion > 8" in mig8 and 'return;' in mig8)
check('migration_v8_no_rollback','rollback is intentionally disabled' in mig8)
check('migration_v9_number',"'slug' => 'rental'" in mig9 and "'version' => 9" in mig9)
check('migration_v9_newer_guard',"$canonicalVersion > $specification['version']" in mig9)
check('migration_v10_number',"'slug' => 'rental'" in mig10 and "'version' => 10" in mig10)
check('migration_v10_immutable_guard','different immutable definition' in mig10)
check('migration_v10_no_rollback','rollback is intentionally disabled' in mig10)
check('migration_v11_number',"'slug' => 'rental'" in mig11 and "'version' => 11" in mig11)
check('migration_v11_immutable_guard','different immutable definition' in mig11)
check('migration_v11_no_rollback','rollback is intentionally disabled' in mig11)
check('migration_v12_number',"'slug' => 'rental'" in mig12 and "'version' => 12" in mig12)
check('migration_v12_optional_annex_summary','جميع الملاحق يختارها المستخدم فقط' in mig12)
check('migration_v12_immutable_guard','different immutable definition' in mig12)
check('migration_v12_no_rollback','rollback is intentionally disabled' in mig12)
