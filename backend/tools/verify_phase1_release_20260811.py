...
check('home_three_paths', all(x in home for x in ['إنشاء عقد بنفسي','إنشاء عقد بواسطة محامي المكتب','استشارة قانونية']))
check('old_creation_modal_removed', not (root/'frontend/src/components/home/CreationModeModal.tsx').exists() and 'CreationModeModal' not in '\n'.join(p.read_text(errors='ignore') for p in (root/'frontend/src').rglob('*.tsx')))
check('contract_catalog_search', 'ابحث عن عقد' in cat and 'variantMeta' in cat)
...
court_values=['القاهرة','شمال القاهرة','جنوب القاهرة','القاهرة الجديدة','شمال الجيزة','جنوب الجيزة','الإسكندرية','طنطا','دمنهور','كفر الشيخ','المنصورة','الزقازيق','بنها','شبين الكوم','بورسعيد','الإسماعيلية','السويس','دمياط','بني سويف','الفيوم','أسيوط','سوهاج','قنا','الأقصر','أسوان','البحر الأحمر','الوادي الجديد','شمال سيناء','جنوب سيناء','مرسى مطروح']
check('court_list_complete', all(v in common for v in court_values), f'{sum(v in common for v in court_values)}/{len(court_values)}')

for fn, expected_v in [('rental.json',7),('apartment_sale.json',6),('freelancer.json',6)]:
    ...
    # Sale/freelancer expose an explicit court selector. Rental source contracts instead fix local jurisdiction by the property location.
    if fn in {'rental.json','apartment_sale.json'}:
        legal_text=' '.join(c.get('bodyAr','') for c in d.get('legalClauses',[]))
        legal_text=re.sub(r'\s+',' ',legal_text)
        no_court_fields=all(not any('competent_court' in key for key in fields if key) for _,fields in all_fields)
        property_court=('يقع في دائرتها' in legal_text and 'العقار' in legal_text)
        check(f'{fn}_court_by_property', no_court_fields and property_court, str([(k,[x for x in f if x and 'competent_court' in x]) for k,f in all_fields]))
    else:
        check(f'{fn}_court_all_variants', all(any('competent_court' in key for key in fields if key) for _,fields in all_fields), str([(k,[x for x in f if x and 'competent_court' in x]) for k,f in all_fields]))
...
check('requests_customer_title_generalized', 'طلباتي' in requests_page and 'استشاراتي' not in requests_page)
