...
check('manual_annex_never_autofills_shared_fields', all(marker not in worker for marker in ['manualAnnexContext','manualAnnexFieldValue','manualAnnexLabelValue']))
check('worker_renders_only_user_selected_annexes', "$variant['requiredAnnexKeys']" not in worker and 'Every annex is optional' in worker and "array_map('strval',$selected)" in worker)
check('php_engine_skips_annex_only_validation', 'manualFillAnnex are printed as blank templates and excluded from wizard validation' in engine)
...
check('all_variants_have_zero_required_annexes', all('requiredAnnexKeys' not in variant for definition in (rent,sale,free) for variant in definition.get('variants',[])))
check('all_optional_clauses_are_never_mandatory', all('requiredWhen' not in item for definition in (rent,sale,free) for item in definition.get('optionalClauses',[])))
check('website_all_six_annexes_user_selectable', website.get('allowedOptionalClauseKeys')==['website_scope_annex','website_technical_annex','website_project_data_annex','website_delivery_annex','website_sla_annex','website_future_development_annex'])
