...
export default function WorkRequestDetailsPage() {
  ...
  useEffect(() => {
    if (!data || !["consultation","contract_review"].includes(data.requestType) || !canSchedule) {
  ...
  const meetingSupported = Boolean(data && ["consultation","contract_review"].includes(data.requestType));
  const meetingAllowed = Boolean(data && meetingSupported && !["awaiting_payment","completed","cancelled"].includes(data.status));
  const deliverableAllowed = Boolean(data && (data.requestType === "consultation" ? ["new","assigned","meeting_scheduled","in_progress"].includes(data.status) : ["in_progress","revision_requested","client_review"].includes(data.status)));
  ...
  async function scheduleMeeting(event: FormEvent<HTMLFormElement>) {
    ...
    let scheduledAt = "";
    if (data && ["consultation","contract_review"].includes(data.requestType)) {
