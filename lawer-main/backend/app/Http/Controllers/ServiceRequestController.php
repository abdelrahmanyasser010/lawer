...
final class ServiceRequestController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        if($this->workflow->supportsBooking($type)&&empty($data['availabilitySlotKey']))throw new ApiException(422,$type==='consultation'?'اختر موعدًا متاحًا للاستشارة':'اختر موعدًا متاحًا لمناقشة تقرير مراجعة العقد','SERVICE_SLOT_REQUIRED');
        ...
        if($type==='consultation'){
            $priceKey='services.consultation.fee_egp';$totalPriceKey=$priceKey;$serviceTotalPriceEgp=$this->settingAmount($priceKey);$expectedPaymentEgp=$serviceTotalPriceEgp;$serviceDepositEgp=$expectedPaymentEgp;
        }elseif($type==='contract_review'){
        ...
        $serial=$this->serial($type);$status=$expectedPaymentEgp>0?'awaiting_payment':'new';$paymentStage=$type==='consultation'?'single':($expectedPaymentEgp>0?'deposit':'working');
    ...
    public function status(Request $request,int $id)
    {
        ...
        $result=DB::transaction(function()use($request,$id,$data,$auth,$visible){
            ...
            if($this->workflow->supportsBooking((string)$current->request_type)){
                if($data['status']==='cancelled')$this->schedule->setBookingStatus($id,'cancelled');elseif($data['status']==='completed')$this->schedule->setBookingStatus($id,'completed');elseif($current->request_type==='consultation'&&$data['status']==='meeting_scheduled')$this->schedule->setBookingStatus($id,'meeting_scheduled');
            }
    ...
    public function meeting(Request $request,int $id)
    {
        ...
        $result=DB::transaction(function()use($request,$id,$data,$auth){
            ...
            $scheduled=date(DATE_ATOM,strtotime($data['scheduledAt']));$reserved=$this->schedule->reserveAt($id,$data['provider'],$scheduled,'meeting_scheduled');$scheduled=$reserved['slotStart'];$nextStatus=$current->request_type==='consultation'?'meeting_scheduled':$current->status;
    ...
    public function deliverable(Request $request,int $id)
    {
        ...
        $result=DB::transaction(function()use($request,$id,$data,$auth,$isFinal){
            ...
            $meta=$this->json($row->metadata_json);$paymentDue=0.0;$nextStatus=$row->request_type==='consultation'?(string)$row->status:'client_review';
