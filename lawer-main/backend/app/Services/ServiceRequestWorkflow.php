...
final class ServiceRequestWorkflow
{
    private const STAFF_TRANSITIONS = [
        'consultation' => [
            ...
            'new' => ['meeting_scheduled','in_progress','awaiting_client_info','cancelled'],
            'assigned' => ['meeting_scheduled','in_progress','awaiting_client_info','cancelled'],
            'awaiting_client_info' => ['new','meeting_scheduled','in_progress','cancelled'],
            'meeting_scheduled' => ['in_progress','completed','awaiting_client_info','cancelled'],
            'in_progress' => ['completed','awaiting_client_info','cancelled'],
        ],
        'contract_review' => [
            'awaiting_payment' => ['cancelled'],
    ...
    {
        return in_array($type,['consultation','contract_review'],true);
    }

    public function assertMeetingAllowed(string $type, string $status): void
    {
        if (!$this->supportsBooking($type)) throw new ApiException(409,'هذا النوع من الطلبات لا يستخدم جدول المواعيد','MEETING_NOT_SUPPORTED');
        if ($type === 'consultation') {
            if (!in_array('meeting_scheduled',$this->allowedStaffTransitions($type,$status),true) && $status !== 'meeting_scheduled') {
                throw new ApiException(409,'لا يمكن تحديد موعد للاستشارة في حالتها الحالية','MEETING_NOT_ALLOWED',['status'=>$status]);
            }
            return;
        }
    ...
    {
        $allowed = $type === 'consultation'
            ? ['new','assigned','meeting_scheduled','in_progress']
            : ['in_progress','revision_requested','client_review'];
