...
@php
  $formatClauseBody = static function ($body): string {
      ...
      return preg_replace_callback(
          ...
          static fn (array $match): string => ($match[1] ?? '') . (($match[2] ?? '') !== '' ? $match[2] : ($match[3] ?? '')) . ".\u{200E} ",
          $text,
      ) ?? $text;
  };
...
@foreach($clauses as $index=>$clause)
  ...
  <div class="clause">
    ...
    <p>{!! nl2br(e($formatClauseBody($clause['bodyAr']))) !!}</p>
  </div>
  @endif
@endforeach
...
@foreach($annex['clauses'] as $index=>$clause)
  @if(trim((string)($clause['bodyAr'] ?? '')) !== '')
  <div class="clause"><h2>{{ $clause['titleAr'] ?: ('البند '.($index+1)) }}</h2><p>{!! nl2br(e($formatClauseBody($clause['bodyAr']))) !!}</p></div>
  @endif
@endforeach
