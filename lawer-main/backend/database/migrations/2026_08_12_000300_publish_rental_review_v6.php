...
return new class extends Migration {
    public function up(): void
    {
        ...
        if (($definition['slug'] ?? null) !== 'rental') {
            throw new RuntimeException('Canonical rental template definition is required by this migration.');
        }
        $canonicalVersion = (int)($definition['version'] ?? 0);
        // Fresh installs may already ship a newer immutable canonical definition.
        // In that case v6 is intentionally skipped and the later publication migration handles it.
        if ($canonicalVersion > 6) {
            return;
        }
        if ($canonicalVersion !== 6) {
            throw new RuntimeException('Canonical rental v6 definition is required by this migration.');
        }
        ...
        DB::transaction(function () use ($raw): void {
            ...
            if ($version) {
                ...
                if ($existing != $incoming) {
                    throw new RuntimeException('Rental template version 6 already exists with a different immutable definition.');
                }
                ...
                $versionId = DB::table('template_versions')->insertGetId([
                    ...
                    'status' => 'published',
                    'definition_json' => $raw,
                    'change_summary' => 'مراجعة عقود الإيجار السكني والتجاري والإداري ومحضر الاستلام والجرد: مطابقة الحقول الإلزامية والاختيارية، الإلزام الشرطي، الأجرة والتأمين والزيادة، الإنهاء المبكر، الضمانات، الإخطارات، الشهود، المحكمة بحسب موقع العقار، ومحضر جرد فارغ شرطي.',
                    'legal_reference' => 'Residential, commercial, administrative lease PDFs and handover/inventory report reviewed 2026-08-12',
                    'effective_from' => now(),
                    'published_at' => now(),
                    'created_at' => now(),
                    'updated_at' => now(),
