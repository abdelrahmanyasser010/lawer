...
return new class extends Migration {
    public function up(): void
    {
        ...
        if ($canonicalVersion !== 7) {
            throw new RuntimeException('Canonical rental v7 definition is required by this migration.');
        }
        ...
        DB::transaction(function () use ($raw): void {
            ...
            if ($version) {
                ...
                if ($existing != $incoming) {
                    throw new RuntimeException('Rental template version 7 already exists with a different immutable definition.');
                }
                ...
                $versionId = DB::table('template_versions')->insertGetId([
                    ...
                    'status' => 'published',
                    'definition_json' => $raw,
                    'change_summary' => 'إضافة مهلة سماح صريحة ومطلوبة في عقد الإيجار السكني بين تاريخ استحقاق الأجرة وبدء احتساب التعويض الاتفاقي عن التأخير، مع ربط الحقل بالتحقق والـPDF وإزالة الإحالة الغامضة إلى مهلة غير محددة.',
                    'legal_reference' => 'Residential lease payment-grace clarification approved 2026-08-12',
                    'effective_from' => now(),
                    'published_at' => now(),
                    'created_at' => now(),
                    'updated_at' => now(),
