...
final class DatabaseWorkflowSmokeTest extends TestCase
{
    ...
    {
        $this->setSetting('services.consultation.fee_egp', 300);
        ...
        $created->assertCreated()
            ...
            ->assertJsonPath('data.paymentAmountEgp', 300);
        ...
        $auth->getJson('/api/v1/service-requests/'.$requestId)
            ...
            ->assertJsonPath('data.requestType', 'consultation')
            ->assertJsonPath('data.status', 'awaiting_payment')
            ->assertJsonPath('data.bookingStatus', 'pending_payment')
            ->assertJsonPath('data.expectedPaymentEgp', 300);
    }

    public function test_consultation_channels_catalog_and_payment_instructions_gate(): void
    {
        ...
        $catalog->assertJsonStructure(['data' => [
            'services' => ['consultationFeeEgp', 'contractReviewFeeEgp', 'contractReviewDepositEgp'],
    ...
    public function test_expired_booking_persists_and_client_can_rebook_before_payment(): void
    {
        ...
        $created = $client->postJson('/api/v1/service-requests', [
            'requestType' => 'consultation',
            'title' => 'استشارة انتهاء مهلة الحجز',
            'description' => 'اختبار تثبيت حالة انتهاء الحجز ثم السماح للعميل باختيار موعد جديد.',
            'communicationChannel' => 'whatsapp',
            'availabilitySlotKey' => $slot['slotKey'],
        ])->assertCreated();
        $requestId = (int) $created->json('data.id');
    ...
    public function test_payment_approval_cannot_revive_cancelled_service_request(): void
    {
        ...
        $created = $client->postJson('/api/v1/service-requests', [
            'requestType' => 'consultation',
            'title' => 'استشارة ستلغى بعد رفع الإيصال',
    ...
    {
        $response = $auth->getJson('/api/v1/consultation-availability?channel='.$channel)->assertOk();
        ...
        $this->fail('No consultation slot is available. Run migrations/seeds or configure a future schedule window for database workflow tests.');
    }
