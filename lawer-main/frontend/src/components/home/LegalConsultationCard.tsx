...
export default function LegalConsultationCard() {
  ...
  const { paymentAccess, paymentVerified, paymentCashNumber, requireVerified } = usePaymentAccess("/#consultation");
  ...
  const requiredAmount = consultationFee;
    ...
    <section id="consultation" className="mt-14 scroll-mt-24 overflow-hidden rounded-3xl border border-slate-800 bg-[#00102e] p-6 text-white shadow-2xl sm:p-9">
      <div className="grid gap-8 lg:grid-cols-[.82fr_1.18fr]">
        ...
        <div className="space-y-5 rounded-2xl border border-white/10 bg-slate-950/60 p-5 sm:p-6">
          ...
          {requiredAmount > 0 && !paymentVerified && <button type="button" onClick={() => void requireVerified()} className="rounded-xl border border-[#986410]/30 bg-[#986410]/10 p-4 text-xs font-black text-[#d9a84e]">{paymentAccess === "unverified" ? "أكد بريدك الإلكتروني لعرض تعليمات الدفع" : "سجّل الدخول وأكد بريدك لعرض تعليمات الدفع"}</button>}
          {requiredAmount > 0 && paymentVerified && <>
            ...
            <label className={`block rounded-xl border border-dashed border-[#986410]/50 bg-[#986410]/10 p-4 text-center transition hover:bg-[#986410]/20 ${catalogLoading || !cashNumber ? "cursor-not-allowed opacity-60" : "cursor-pointer"}`}>
              ...
              <span className="mt-2 block text-xs font-black">{receipt?.name || `رفع إثبات دفع الاستشارة — ${requiredAmount.toLocaleString("ar-EG")} ج.م`}</span>
            </label>
          </>}
