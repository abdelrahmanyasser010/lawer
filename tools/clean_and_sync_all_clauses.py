...
def clean_arabic_text(text: str) -> str:
    ...
    text = re.sub(r'االأ', 'الأ', text)
    text = re.sub(r'االإ', 'الإ', text)
    text = re.sub(r'االآ', 'الآ', text)
    text = re.sub(r'األ', 'الأ', text)
    text = re.sub(r'اإل', 'الإ', text)
    text = re.sub(r'اآل', 'الآ', text)
    text = re.sub(r'اال', 'الا', text)
    text = re.sub(r'إنوجدت', 'إن وجدت', text)
    text = re.sub(r'إذارفعت', 'إذا رفعت', text)
    text = re.sub(r'\bت\s+عمد\b', 'تعمد', text)
...
for data in [rental_data, sale_data, freelance_data]:
    for c in data.get('legalClauses', []):
        if 'titleAr' in c and c['titleAr']:
            c['titleAr'] = clean_arabic_text(c['titleAr'])
